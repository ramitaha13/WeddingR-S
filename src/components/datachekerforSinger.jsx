...
const DateCheckerForSinger = () => {
  ...
  useEffect(() => {
    const fetchSingerName = async () => {
      try {
        ...
        const singerInfoRef = ref(db, `Singer/${hallId}`);
        const snapshot = await get(singerInfoRef);

        if (snapshot.exists()) {
          setSingerName(snapshot.val().name);
        } else {
          setError("رقم الفنان غير صالح");
        }
      } catch (err) {
        console.error("خطأ في جلب اسم الفنان:", err);
      }
    };
  ...
  const checkDate = async () => {
    ...
    try {
      const db = getDatabase();
      const singerRef = ref(db, `SingerNames/${hallId}`);
                ...
                {result.isBooked ? (
                  <>
                    ...
                    <div className="text-right">
                      <p>اسم الزبون: {result.details.name}</p>
                    </div>
                  </>
                ) : (
                  <div className="text-right">
                    <p className="font-bold">
                      يمكنك الحجز مع {singerName} في هذا التاريخ
                    </p>
                  </div>
                )}
              </div>
...
export default DateCheckerForSinger;
